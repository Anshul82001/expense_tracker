...
# CLASS 1: Expense
# __init__(self, expense_id, category, amount, description)
# __repr__
class Expense:
    def __init__(self,expense_id:int, category:str, amount:int, description:str, expense_date:date=None)->None:
...
#   self.expenses = []
#   self._next_id = 1   ← auto-increments with each expense added
...
# Expected usage at end of Stage 1:


tracker = ExpenseTracker("Rahul")
# Test valid expense
try:
    tracker.add_expense("Food", 1150, "Lunch", date(2026, 6, 1))
except (InvalidAmountError, InvalidCategoryError) as e:
    print(f"Error: {e}")

# Test invalid amount
try:
    tracker.add_expense("Food", -100, "Lunch", date(2026, 6, 1))
except InvalidAmountError as e:
    print(f"Caught: {e}")

# Test empty category
try:
    tracker.add_expense("", 250, "Lunch", date(2026, 6, 1))
